...
class First_Multiple_Guess(tk.Frame):
    ...
    def __init__(self, file_path, parent=None):
        ...
        self.parent= parent
        self.parent.title('FBLA 2021 Coding and Programming')
        tk.Frame.__init__(self, self.parent)
        self.pack(expand='yes')       
        self.canvas= tk.Canvas(self)
        self.canvas.config(width= 1920, height= 1080, bg='gray')
        xbar= tk.Scrollbar(self, orient='horizontal',)        
        xbar.config(command= self.canvas.xview)        
        self.canvas.config(yscrollcommand= xbar.set)                           
        xbar.pack(side='bottom',padx=2,pady=2)
    ...
    def check_ans(self, letter):
        ...
        if sol.strip() == letter:
            ...
            if self.total_count == self.length_quest:
                ...
                self.count +=1
                self.canvas.after(2000, self.finish)
            else:
                self.frame.place_forget()
    ...
    def finish(self):
        text= 'Great Job! Your score was: {:.2%}'.format(self.win_count/self.total_count)
        self.frame.place_forget()
        self.solution_var.set(text)
        self.question_var.set(' ')
